...
export class ChatController {
  ...
  @Get()
  findAll(@GetUser() user: User, @Query('page') page = 1, @Query('limit') take = 10) {
    return this.chatService.getRecentChats(user.profile, page, take);
  }

  @Post()
  createGroupChat(@GetUser() user: User, @Body() dto: CreateGroupChatDTO) {
  ...
  async getMessages(
    ...
    if (!isUserInGroupChat) throw new UnauthorizedException('You are not a member of this chat');
  ...
  @Get('with/:id')
  async getProfileChats(@GetUser() user: User, @Param('id') id: string) {
    return this.chatService.findChat([user.profile.id, id], true);
  }

  @Patch(':id')
  async updateGroupChat(@GetUser() user: User, @Body() updateGroupChatDTO: UpdateGroupChatDTO) {
  ...
  async updateMember(@GetUser() user: User, @Param('chatId') chatId: string, @Body() updateMemberDto: UpdateMemberDto) {
  ...
  async typing(
    ...
    return this.chatService.typing(chatId, user.profile.id);
  }
  ...
  @Delete(':id')
  async deleteChat(@GetUser() user: User, @Param('id') id: string) {
