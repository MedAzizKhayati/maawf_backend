...
export class ChatService extends GenericsService<GroupChat, GroupChat, GroupChat> {
  ...
  async updateGroupChat(updateDTO: UpdateGroupChatDTO): Promise<GroupChat> {
    ...
    const members = updateDTO.newMembers.map(member => this.profileRepository.create({ id: member.id }));
    ...
    const groupChatToProfiles = [
      ...
      ...members.map(member => {
        const groupChatToProfile = new GroupChatToProfile();
        groupChatToProfile.profile = member;
        groupChatToProfile.encryptedSymmetricKey = updateDTO.newMembers.find(
          memberDto => memberDto.id === member.id,
  ...
  async getMessages(groupChat: GroupChat | string, page = 1, take = 10): Promise<Message[]> {
    ...
    return this.messageRepository
      .find(
        addPaginationToOptions<Message>(
          {
            where: {
              groupChat: {
                id,
  ...
  async sendMessage(
    ...
    if (!message.text) delete messageEntity.data.text;

    const [messageResult, concernedProfiles] = await Promise.all([
      this.messageRepository.save(messageEntity),
      this.getConcernedProfiles(groupChat).then(profiles => profiles.map(profile => profile.id)),
    ]);
  ...
  async markAsRead(message: Message | string, profile: Profile | string): Promise<Message> {
    ...
    const concernedProfiles = await this.getConcernedProfiles(message.groupChat).then(profiles =>
  ...
  async getConcernedProfiles(groupChat: GroupChat | string): Promise<Profile[]> {
    ...
    return this.groupChatToProfileRepo
      ...
      .then(crToProfiles => crToProfiles.map(crToProfile => crToProfile.profile));
  }
  ...
  async findOne(id: string, profile?: Profile | string): Promise<GroupChat> {
    ...
    if (!groupChat) throw new NotFoundException('Chat not found');
  ...
  async updateMember(groupChat: GroupChat | string, updateMemberDto: UpdateMemberDto): Promise<GroupChatToProfile> {
    const id = typeof groupChat === 'string' ? groupChat : groupChat.id;
    ...
    if (!groupChatToProfile) throw new NotFoundException('This user is not a member of this group chat');
  ...
  async deleteGroupChat(groupChat: GroupChat | string, profile: Profile | string): Promise<void> {
    groupChat =
      typeof groupChat === 'string' ? await this.groupChatRepository.findOneByOrFail({ id: groupChat }) : groupChat;
    const isUserAdmin = await this.isUserAdminOfGroupChat(groupChat, profile);
    if (!isUserAdmin) throw new ForbiddenException('You are not an admin of this group chat');
    if (groupChat.isPrivate) throw new ForbiddenException('You cannot delete a private chat');
  ...
  async typing(groupChat: GroupChat | string, profile: Profile | string): Promise<void> {
    const id = typeof groupChat === 'string' ? groupChat : groupChat.id;
    const profileId = typeof profile === 'string' ? profile : profile.id;
